...
#R=4.3.2
.libPaths('/cluster/tufts/dopmanlab/Jacob/packages_r_4.3.2/')
.libPaths()
...
#install.packages("multcompView", lib='/cluster/tufts/dopmanlab/Jacob/packages_r_4.3.2/')
#BiocManager::install("DEGreport", lib='/cluster/tufts/dopmanlab/Jacob/packages_r_4.3.2/')


setwd("/cluster/tufts/dopmanlab/Jacob/brbseq/")

###################################################
counts_d2 <- read.table("./umi.counts_d2_v2.txt",
                        header=TRUE, row.names = 1)

counts_d3 <- read.table("./umi.counts_d3_v2.txt",
                        header=TRUE, row.names = 1)
...
counts[which(rownames(counts) == "LOC135077073"),]

#pdf receptor like
counts[which(rownames(counts) == "LOC135087024"),]
sum(counts[which(rownames(counts) == "LOC135087024"),])
...
#        aes(x = treatment, y = depth/1e6, color = bkgrnd)) + 
#   geom_boxplot() + geom_point(position=position_jitterdodge(dodge.width=0.8)) +
...
pca
#ggsave("pca_expression_12L12D.png", plot = pca, width = 6, height = 4, dpi = 800, bg = 'white')
...
for (i in 1:nrow(rhyLimmaSummary_cg)) {
  ...
  result2$plot <- result2$plot + theme_bw() +
    ...
    scale_colour_manual(values = c('b_nd' = "#00BFC4", 'fh_long' = "#F8766D"),
                        labels = c('fh_long' = "Univoltine",
                                   'b_nd' = "Bivoltine")) +
    #geom_vline(xintercept = result2$summary[11,2], color = "#00BFC4", linetype = 'solid' ) +
    #geom_vline(xintercept = result2$summary[12,2], color = "#F8766D", linetype = 'solid' ) +
...
library(ggpubr)
#torso
cg_plots$torso$layers[[5]] <- NULL
cg_plots$torso = cg_plots$torso + scale_y_continuous(seq(-5,5,by=5),
                                                     name = expression("Expression"*"(log"[2]*"CPM)"),
                                                     expand=c(0,0),
                                                     limits=c(-7.5,7.5)) +
  annotate("rect", xmin=0, xmax=12, ymin=-7.5, ymax=7.5, alpha=0.2, fill="gray")

cg_plots$ptth$layers[[5]] <- NULL
cg_plots$ptth$layers[[1]]$aes_params$linetype = 'longdash'
cg_plots$ptth$layers[[2]]$aes_params$linetype = 'longdash'
cg_plots$ptth = cg_plots$ptth + scale_y_continuous(seq(0,6,by=2),
                                                     name = expression("Expression"*"(log"[2]*"CPM)"),
                                                     expand=c(0,0),
                                                     limits=c(-1,7)) +
...
cg_plots$torso = cg_plots$torso +
  annotate("segment",
           x = cg_circacompare$torso[11,2],
           xend = cg_circacompare$torso[11,2],
           y = -7.5,
           yend = sum(cg_circacompare$torso[3,2]+cg_circacompare$torso[7,2]),
           colour = "#00BFC4") +
  annotate("segment",
           x = cg_circacompare$torso[12,2],
           xend = cg_circacompare$torso[12,2],
           y = -7.5,
           yend = sum(cg_circacompare$torso[4,2]+cg_circacompare$torso[8,2]),
           colour = "#F8766D") +
  annotate("text", x = 6, y = 5, label = expression(Phi~"= -7.4 hours"), size = 3.5)
  

ptth_torso = ggarrange(cg_plots$ptth + rremove('xlab')+ rremove("ylab"), cg_plots$torso + rremove('xlab') + rremove("ylab"), ncol = 2, common.legend = TRUE, align = 'h')
ptth_torso_final = annotate_figure(ptth_torso, left = textGrob(expression("Expression"*"(log"[2]*"CPM)"), rot = 90, vjust = 1, gp = gpar(cex = 1.3)),
                                   bottom = textGrob("Zeitgeber Time (hours)", gp = gpar(cex = 1.3)))

ggsave("ptth_torso.png", plot = ptth_torso_final, width = 6.66, height = 2, dpi = 800, bg = 'white')
...
#ggsave("clk_expression.png", plot = all_ccgs, width = 7.5, height = 6, dpi = 800, bg = 'white')
...
#tim
cg_plots$tai = cg_plots$tai + scale_y_continuous(seq(-3,3,by=3),
                                                 name = expression("Expression"*"(log"[2]*"CPM)"),
                                                 expand=c(0,0),
                                                 limits=c(-4.5,4.5)) +
  annotate("rect", xmin=0, xmax=12, ymin=-4.5, ymax=4.5, alpha=0.2, fill="gray")
...
#make line for fh_long dashed because this group is not rhythmic
cg_plots$tai$layers[[2]]$aes_params$linetype = 'longdash'
cg_plots$tai
